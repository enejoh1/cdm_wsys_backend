Ext.define('Msys.views.sales.CustomerMgmtView', {
    ...
    initComponent: function() {
       ...
       (buttonToolbar.items).each(function(item,index,length){
		...
		//스토어 생성
        this.createStore('Msys.model.CustomerMgmtModel', gu.pageSize);
	...
	crudHandler: function(type, record) {
		...
		switch(type){
			case 'ADD':
				gm.me().crudText='고객사 등록';
			break;
			case 'EDIT':
				gm.me().crudText='고객사 수정';
                            ...
                            items:[
                                {
									...
									value: (type=='ADD')? '' : record.get('cst_code')
                                },{
                                    xtype:'button',
                                    text:'자동생성',
                                    width:'10%',
									...
									listeners: {
                                        click: function(btn) {
                                            var cst_code = Ext.getCmp('cst_code');
                                            // 다음 고객사코드 가져오기
                                            Ext.Ajax.request({
                                                url: CONTEXT_PATH + '/util.do?method=getAutoCode',
												params:{
													table_name : 'cstmst',
													column_name : 'cst_code',
													first_code : 'CST',
													length : 4
												},
                                    ...
                                    gm.me().store.load(
										{
											callback: function(r,options,success) {
										        if(type=='EDIT'){
													gm.me().vSelectCstmst =  gm.me().store.findRecord('unique_id', unique_id);
												}
											}
										}
									);	
